...
def generate_data(seed=14):
    ...
    # x1 = np.random.uniform(-10, -5, num_samples)  # placeholder
    # x2 = np.random.uniform(5, 10, num_samples)  # placeholder
    ...
    for i in range(len(data.index)):
        ...
        if data['true_clust'][i] == 1:
            data['y1'][i] = 40*x1**2 - 400  + np.random.normal(0, 50)
        elif data['true_clust'][i] == 2:
            data['y1'][i] = -(x1-8)**3  + np.random.normal(0, 50)
        elif data['true_clust'][i] == 3:
            data['y1'][i] = (x1+8) **3 - 20 + np.random.normal(0, 50)
    return data
