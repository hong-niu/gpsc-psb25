...
def generate_data(seed=14):
    ...
    for i in range(len(data.index)):
        ...
        if data['true_clust'][i] == 1:
            data['y1'][i] = 40*x1**2 - 500  + np.random.normal(0, 2) + 10 * (s1 + s2)**2 + 40 * x2**2
        elif data['true_clust'][i] == 2:
            data['y1'][i] = -(x1-8)**3  + np.random.normal(0, 2) + 10 * s2**2 + x2**3
        elif data['true_clust'][i] == 3:
            data['y1'][i] = (x1+8) **3 - 20 + np.random.normal(0, 2) - 10 * s1**2 + x2**3
    return data
...
def main_simul_2(tune = False, visualize=True, seeds=1):
    ...
    for seed in range(14, 14+seeds):
        ...
        data = utils.random_scramble(data, num_clust=3, save_random=True)
        ...
        spectral = SpectralClustering(n_clusters=num_clusters, affinity='nearest_neighbors', n_neighbors=6).fit(data[kmeans_list])
        if tune:
            utils.tune_parameters_spectral(data=data, kmeans_list=kmeans_list, num_clusters=2, print_thresh=0.1)
            #  SPECTRAL: max score = 0.018468635748222037, n_neighbors= 6
        ...
        dbscan = DBSCAN(eps=79, min_samples=27).fit(data[kmeans_list])
        if tune:
            utils.tune_parameters_dbscan(data, kmeans_list)
            #  DBSCAN: max=0.10439518000485269, eps=79, min_samples=27
        ...
        if tune:
            ...
            # GDBSCAN: score = 0.3195627586572281, covar_thresh = 800, spatial_thresh = 5, min_thresh = 0
        ...
        if tune:
            ...
            # SPATIAL HIER: max = 0.2460042240354879, max_neigh=51, max_linkage=average

        from sklearn.cluster import AgglomerativeClustering
        from sklearn.neighbors import kneighbors_graph

        X = data[['long', 'lat']]
        knn_graph = kneighbors_graph(X, 51, include_self=False)
        model = AgglomerativeClustering(linkage='average', connectivity=knn_graph, n_clusters=3)
        ...
        cntr, u, u0, d, jm, p, fpc = cmeans(data[kmeans_list].T, c=3, m=2, error=0.00001, maxiter=1000, init=np.array(cmeans_init).T)# np.array(cmeans_init).T)
        ...
        data_out = utils.permute_clust(data=data_out, clustering='new_clust', clust1=3, clust2=2)
        data_out = utils.permute_clust(data=data_out, clustering='spatial_hier', clust1=1, clust2=3)
        data_out = utils.permute_clust(data=data_out, clustering='spectral', clust1=2, clust2=3)
        data_out = utils.permute_clust(data=data_out, clustering='spectral', clust1=1, clust2=3)
        data_out = utils.permute_clust(data=data_out, clustering='spectral', clust1=2, clust2=3)
