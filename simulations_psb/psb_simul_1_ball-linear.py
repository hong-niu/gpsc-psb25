...
def main_simul_1(tune=False, visualize=True, seeds=1):
    ...
    for seed in range(14, 14 + seeds):
        ...
        spectral = SpectralClustering(n_clusters=num_clusters, affinity='nearest_neighbors', n_neighbors=11).fit(data[kmeans_list])
        if tune:
            utils.tune_parameters_spectral(data=data, kmeans_list=kmeans_list, num_clusters=num_clusters, print_thresh=0.1)
            #   SPECTRAL: max score = 0.23812266210409097, n_neighbors= 11
        ...
        if tune:
            ...
            #   DBSCAN: max=0.28952576066074737, eps=3, min_samples=77
        ...
        if tune:
            utils.tune_parameters_gdbscan(data=data, cov_thresh=200, spatial_thresh=20, min_thresh=0, print_thresh=0.15)
            # GDBSCAN: score = 0.2714237434436845, covar_thresh = 3, spatial_thresh = 13, min_thresh = 0
        ...
        if tune:
            utils.tune_parameters_spatial_hier(data=data, s_list=s_list, num_clusters=num_clusters, n_neighbors=75, print_threshold=0.1)
            # SPATIAL HIER: max = 0.1588695050443135, max_neigh=1, max_linkage=ward

        X = data[['long', 'lat']]
        knn_graph = kneighbors_graph(X, n_neighbors=1, include_self=False)
        ...
        if visualize:
            ...
            utils_visual.plot_spatial_data_matrix_simulation_3x3_comparisons_2(data_out, x_list, y_list, ax_right=ax_right, ax_left=ax_left, wspace=-.4, hspace=.00)
